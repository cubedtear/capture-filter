// Taken from Zig std.io.bit_reader
// SPDX-License-Identifier: MIT

// Changes from Zig 0.13:
// - Changed endian to not be comptime
// - Add a "skipBytes" function
// - Add a "underlyingReader" function
...
pub fn BitReader(comptime ReaderType: type) type {
    return struct {
        ...
        pub fn init(endian: std.builtin.Endian, forward_reader: ReaderType) Self {
            return Self{
                .endian = endian,
        ...
        pub fn skipBytes(self: *Self, byte_count: usize) !void {
            if (self.bit_count > 0) {
                return error.NotByteAligned;
            }

            return try self.forward_reader.skipBytes(byte_count, .{});
        }
        ...
        pub fn underlyingReader(self: *Self) ReaderType {
            return self.forward_reader;
        }
    };
}
...
) BitReader(@TypeOf(underlying_stream)) {
    return BitReader(@TypeOf(underlying_stream)).init(endian, underlying_stream);
}
